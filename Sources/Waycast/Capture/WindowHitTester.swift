...
//  Waycast
//
//  Ported from Mio (github.com/iSoldLeo/Mio, GPL-3.0) — live Quartz window
//  hit-test. Window RECOGNITION is Quartz-only and live: each hover re-queries
//  CGWindowListCopyWindowInfo for the frontmost selectable window under the
//  cursor, skipping our own overlay panels. This is what makes window hover
//  work even though the frozen overlay sits in front of everything.
//
...
struct WindowHitTestResult: Equatable {
...
enum WindowHitTester {
    ...
    /// `skipSelfWindows` skips our own PID so the live overlay panels (which
    /// are frontmost while hovering) are ignored and the real window behind
    /// is found.
