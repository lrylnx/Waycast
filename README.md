...
- **网速**：状态栏实时显示所有物理网卡（Wi-Fi / 有线 / 个人热点）的合计上下行速率，每秒刷新。图标里**上下两行**，上排下行 `↓`、下排上行 `↑`。
...
- **图标宽度**：状态栏图标每秒重绘，宽度一变整排菜单栏图标都会左右抖。因此这几个图标都用 SF Mono 并按固定字符数排版——网速**上下两行**、每行恒为 5 字符（`↓1.2M` / `↑340K`），CPU 温度恒为 5 字符（` 42°C`）。实测 SF Mono 每个字符宽度完全一致（11pt 下恒为 6.80pt），且**字重不影响字符宽度**，所以加粗不占额外位置。最终占位：网速 **36pt 宽 × 20pt 高**，CPU 温度 **36pt 宽 × 9pt 高**，宽度都恒定不抖。
