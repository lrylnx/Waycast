...
一款 macOS 菜单栏效率工具，把**环形应用切换、截图标注、贴图、Spotlight 式搜索、剪贴板历史、输入法锁定、状态栏系统监控（内存水位 / 网速 / CPU 温度）**整合进一个常驻状态栏的应用。无 Dock 图标，随叫随到。
...
| 状态栏图标 | 状态栏菜单 / 设置 | 内存水位、实时网速、CPU 温度三选一，或回到默认闪电 |
...
- **CPU 温度**：取所有 CPU 核心里最高的那一路，每秒刷新。低于 75°C 跟随菜单栏明暗自动取色；75–90°C 转橙、≥90°C 转红。

三种读数都只在**当前被选中时**才启动自己的 1Hz 定时器，没启用的完全不采样、不重绘。实测（macOS 26 / Apple Silicon）开启任一读数时进程 CPU 约 **0.03%**，关闭即归零。

### 实现备注

- **CPU 温度**：Apple Silicon 上 `AppleSMC` 已没有可用的 CPU 温度键（Intel 时代的 `TC0P` 之类整批消失），真实温度在 IOHID 的 AppleVendor 温度传感器里（`PMU tdie0`–`PMU tdie10`）。走的是 **IOKit 的私有接口**，用 `dlsym` 动态解析：万一将来某个系统版本删掉它，应用照常启动，只是这个图标显示 `—°C`。**不需要 root，也不需要任何新权限。**
...
- **图标宽度**：状态栏图标每秒重绘，宽度一变整排菜单栏图标都会左右抖。因此这几个图标都用 SF Mono 并按固定字符数排版——网速**上下两行**、每行恒为 5 字符（`↑340K` / `↓1.2M`），CPU 温度恒为 5 字符（` 42°C`）。实测 SF Mono 每个字符宽度完全一致（11pt 下恒为 6.80pt），且**字重不影响字符宽度**，所以加粗不占额外位置。最终占位：网速 **36pt 宽 × 20pt 高**，CPU 温度 **36pt 宽 × 9pt 高**，宽度都恒定不抖。
- **字重与对齐**：文字图标统一用 SF Mono **`.heavy`**。实测把所有"加粗"做法都渲染出来扫像素、算墨迹覆盖率（墨迹像素数 ÷ 墨迹包围盒面积）：11pt bold 是 0.358、**11pt heavy 是 0.438**、11pt bold + `strokeWidth` -1.2 是 0.398，而 **12pt bold 仍是 0.358** —— 加大字号只放大整体、几乎不加粗笔画，还会让两行网速从 20pt 涨到 22pt 顶满菜单栏。所以选 `.heavy`：加粗 22%，尺寸分毫不变。水平方向，**单行**（温度）按**墨迹**居中 —— 温度是定宽 5 字符、位数不同时前面补空格，按字符格子对齐会让墨迹随数字位数在图标里左右滑动（实测 ` 42°C` 右边只剩 1pt、左边空 7.8pt）；**多行**（网速）则按**字符格子**对齐，两行箭头才不会错位。
...
- **网速为什么竖着排**：并排写（`↓1.2M↑340K`）要横占 10 个字符 ≈ 70pt，把右边一排菜单栏图标挤走一大截；拆成上下两行后宽度直接减半到 36pt。代价是高度——11pt 字体按**自然行距**排两行需要 30pt，超过菜单栏的 22pt（`NSStatusBar.system.thickness`）。所以图标不是交给 `NSAttributedString` 去排版，而是用 CoreText 的 `.useGlyphPathBounds` 量出每行**字形墨迹**的实际高度（数字+箭头约 8pt），只按墨高堆叠、行间留 2pt，两层合计 20pt，稳稳落在菜单栏里。
